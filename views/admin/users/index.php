...
<script>
        ...
        fetch('<?= url('admin/users/update') ?>', {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Помилка!',
                        text: data.message || 'Помилка при оновленні користувача',
                        confirmButtonColor: '#667eea'
                    });
                }
            })
            .catch(error => {
                Swal.fire({
                    icon: 'error',
                    title: 'Помилка!',
                    text: 'Помилка при оновленні користувача',
                    confirmButtonColor: '#667eea'
                });
            });
    });
    ...
    function deleteUser(userId) {
        ...
        Swal.fire({
            ...
            text: `Ви дійсно хочете видалити користувача "${userData.first_name} ${userData.last_name}"?`,
            icon: 'warning',
            ...
            <p class="text-warning"><i class="fas fa-exclamation-triangle"></i> Ця дія незворотна!</p>
        `
        }).then((result) => {
            ...
            fetch('<?= url('admin/users/delete') ?>', {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Помилка!',
                            text: data.message || 'Помилка при видаленні користувача',
                            confirmButtonColor: '#667eea'
                        });
                    }
                })
                .catch(error => {
                    Swal.fire({
                        icon: 'error',
                        title: 'Помилка!',
                        text: 'Помилка при видаленні користувача',
                        confirmButtonColor: '#667eea'
                    });
                });
        });
    ...
    function showNotification(type, message) {
        // Замінюємо на SweetAlert для консистентності
        if (type === 'success') {
            Swal.fire({
                icon: 'success',
                title: 'Успіх!',
    ...
    // Валідація форми створення користувача
            ...
            createForm.addEventListener('submit', function(e) {
                ...
                if (firstName.trim().length < 2 || lastName.trim().length < 2) {
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Помилка валідації',
                        text: 'Ім\'я та прізвище повинні містити мінімум 2 символи',
                        confirmButtonColor: '#667eea'
                    });
                    return;
                }
    ...
    // Додаткова валідація для форми створення
    document.getElementById('email').addEventListener('input', function() {
        ...
        if (email && !emailRegex.test(email)) {
            this.classList.add('is-invalid');
            this.setCustomValidity('Введіть коректну email адресу');
        } else {
            this.classList.remove('is-invalid');
    ...
    document.getElementById('password').addEventListener('input', function() {
        ...
        if (password && password.length < 6) {
            this.classList.add('is-invalid');
            this.setCustomValidity('Пароль повинен містити мінімум 6 символів');
        } else {
            this.classList.remove('is-invalid');
            this.setCustomValidity('');
        }
    ...
    // Очищення форм при закритті модальних вікон
